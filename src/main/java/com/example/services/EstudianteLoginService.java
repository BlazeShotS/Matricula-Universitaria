...
public class EstudianteLoginService {
    ...
    public List<MatriculaResponse> procesarMatricula(MatriculaRequest request) {
        ...
        // Quitar duplicados del request, LISTA DE LOS IDS DE SECCIONES QUE MANDO EL USUARIO ejemplo asi [7, 16, 20 , 12]
        List<Integer> idsUnicos = request.id_secciones().stream().distinct().toList();

        var estudiante = estudianteSistemaRepository.findById(request.codigo_estudiante()).orElseThrow(() -> new IllegalArgumentException("Estudiante no encontrado: " + request.codigo_estudiante()));

        // Traer secciones en lote, compara las secciones de la base de datos con las solicitadas
        ...
        //recorre todas las secciones que el estudiante quiere matricularse y realiza una serie de validaciones y acciones para registrar la matrícula
        for (Seccion seccion : secciones) {
            Integer idSeccion = seccion.getIdSeccion();

            // Si ya está matriculado, usa continue para saltar esa sección y pasar a la siguiente y ya no descontar cupos ni volver a agregar
